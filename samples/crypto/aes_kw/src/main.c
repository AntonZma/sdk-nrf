...
#if defined(CONFIG_AES_KW_SAMPLE_SHOW_KWP)
#define APP_ALGORITHM_USED	"AES-KWP"
#else
#define APP_ALGORITHM_USED	"AES-KW"
#endif /* CONFIG_AES_KW_SAMPLE_SHOW_KWP */
...
int crypto_finish(void)
{
	...
	/* Destroy the KEK key handle */
	status = psa_destroy_key(kek_key_id);
	if (status != PSA_SUCCESS) {
		LOG_INF("psa_destroy_key for KEK failed! (Error: %d)", status);
		return APP_ERROR;
	}
...
int import_kek(void)
{
	...
	status = psa_import_key(&key_attributes, kek, sizeof(kek), &kek_key_id);
	if (status != PSA_SUCCESS) {
		LOG_INF("psa_import_key failed (KEK)! (Error: %d)", status);
		return APP_ERROR;
	}
	...
	LOG_INF("KEK key imported successfully!");
...
int wrap_key(void)
{
	...
	status = psa_wrap_key(kek_key_id, wrap_alg, key_id,
...
int unwrap_key(void)
{
	...
	status = psa_unwrap_key(&key_attributes, kek_key_id, unwrap_alg,
	...
	LOG_INF("Unwrapped key size is %sexpected: %d bytes",
		(key_len == sizeof(key)) ? "" : "NOT ", key_len);
