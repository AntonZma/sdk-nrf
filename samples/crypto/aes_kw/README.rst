...
The AES KW sample demonstrates how to use the :ref:`PSA Crypto API <ug_psa_certified_api_overview_crypto>` to perform key wrapping and unwrapping operations using AES Key Encryption Key (KEK).
...
* :kconfig:option:`CONFIG_PSA_WANT_ALG_AES_KW` - Used to enable AES key-wrapping algorithm based on the NIST Key Wrap (KW) mode of a block cipher from among the supported cryptographic operations for :ref:`_ug_crypto_supported_features_key_wrap_algorithms`.
* :kconfig:option:`CONFIG_PSA_WANT_ALG_AES_KWP` - Used to enable AES key-wrapping algorithm based on the NIST Key Wrap with Padding (KWP) mode of a block cipher from among the supported cryptographic operations for :ref:`_ug_crypto_supported_features_key_wrap_algorithms`.
...
1. Initialization:
   ...
   #. A predefined 128-bit or 192-bit AES key (KEK) is imported using :c:func:`psa_import_key` and stored in the PSA crypto keystore.
...
#. Cleanup:
   ...
   a. Both Key Encryption Key (KEK) and unwrapped plain key are removed from the PSA crypto keystore using :c:func:`psa_destroy_key`.
...
.. code-block:: text
   ...
   *** Using Zephyr OS v4.3.99-a7c2a12b9134 ***
   ...
   [00:00:00.002,535] <inf> aes_kw: Importing KEK key...
   [00:00:00.002,563] <inf> aes_kw: KEK key imported successfully!
