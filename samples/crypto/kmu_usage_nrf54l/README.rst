...
The persistent key sample shows how to generate and use various persistent keys using the Platform Security Architecture (PSA) APIs.
Persistent keys are stored in the Key Management Unit (KMU) of the device and retain their value between resets.
The implementation of the PSA ITS API is provided by the Oberon PSA Crypto, which implements the driver wrapper role
and determines which driver to use for each cryptographic operation.
For this sample CRACEN driver is used.
